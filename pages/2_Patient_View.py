...
if st.session_state.recap_script:
    ...
    for sentence in sentences:
        ...
        for name, profile in people_profiles.items():
            # Check if the profile name (e.g., "sarah") is in the sentence (e.g., "...Sarah, your daughter...")
        ...
        if found_person:
            ...
            with col_img:
                ...
                # Check if the photo_url is a local path and if it exists
                if Path(photo_path).exists():
                    st.image(photo_path, width=80, caption=f"{found_person['name']} ({found_person['relationship']})")
                else:
                    # Show placeholder if local file not found or if it's a broken link
                    st.image("https://via.placeholder.com/150", width=80, caption=f"{found_person['name']} ({found_person['relationship']})")
            with col_text:
                st.write(sentence)
        else:
            st.write(sentence)
    ...
    if st.session_state.recap_audio_path:
        ...
        except Exception as e: pass # Suppress warning
