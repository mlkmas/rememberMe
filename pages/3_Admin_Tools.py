...
from src.database import (
    ...
    update_person # Ensure update_person is imported
)
from pathlib import Path
import face_recognition # For face encoding
import numpy as np # For face encoding arrays
...
with col1:
    ...
    medications = get_all_medications() # Fetch medications

    # --- Medication Edit/Delete Logic ---
    def set_edit_state_med(med_id):
        ...
        if med_to_edit:
            ...
            try:
                ...
                st.session_state.med_time = time(8, 0) # Default if parsing fails
            ...
            if schedule_type == 'One-Time':
                sdate_raw = med_to_edit.get('specific_date', None) # Default to None
                if isinstance(sdate_raw, datetime):
                    st.session_state.specific_date = sdate_raw.date()
                     ...
                     st.session_state.specific_date = date.today() # Fallback
    ...
    def clear_form_state_med():
        ...
        keys_to_clear = ['med_name', 'med_dosage', 'med_purpose', 'med_time',
                         'schedule_type', 'days_of_week', 'specific_date']
    ...
    if not medications:
        ...
        for med in medications:
            med_id = str(med.get('_id')) # Ensure med_id is a string
            with st.container(border=True):
                c1a, c2a = st.columns([3, 1])
                with c1a:
                    stype = med.get('schedule_type', 'Daily') # Default schedule type
                ...
                with c2a:
                    if st.button("Edit", key=f"edit_med_{med_id}", use_container_width=True, on_click=set_edit_state_med, args=(med_id,)):
                        pass # Button click handles state change
                    if st.button("Delete", key=f"del_med_{med_id}", type="secondary", use_container_width=True):
                        ...
                        if st.session_state.editing_med_id == med_id:
                            clear_form_state_med() # Clear form if editing deleted item
                        st.rerun()
    st.divider()
    ...
    # Use st.form for better state handling and clear_on_submit
    with st.form(key="med_form", clear_on_submit=True):
        ...
        med_time = st.time_input("Time to Take", value=st.session_state.get('med_time', time(8,0)), step=timedelta(minutes=1)) # time object

        st.subheader("Scheduling Options")
        schedule_options = ["Daily", "Weekly", "One-Time"]
        current_schedule_index = schedule_options.index(st.session_state.get('schedule_type', 'Daily'))
        schedule_type = st.radio("Schedule Type", schedule_options,
                                 index=current_schedule_index, horizontal=True)

        days_of_week_val = None
        specific_date_val = None
        if schedule_type == 'Weekly':
            days_of_week_val = st.multiselect("Select days",
                                            options=["Monday", "Tuesday", "Wednesday", "Thursday", "Friday", "Saturday", "Sunday"],
                                            default=st.session_state.get('days_of_week', []))
        elif schedule_type == 'One-Time':
            specific_date_val = st.date_input("Select date", value=st.session_state.get('specific_date', date.today())) # date object

        # Submit button inside the form
        submitted = st.form_submit_button(submit_button_text_med, type="primary", use_container_width=True)

        if submitted:
            if not all([med_name, med_dosage, med_purpose]): # Basic validation
                st.error("Please fill out Medication Name, Dosage, and Purpose.")
            else:
                time_str = med_time.strftime("%I:%M %p") # Format time object to string
                # Combine date and time for MongoDB if specific_date is used
                final_specific_date = None
                if schedule_type == 'One-Time' and specific_date_val:
                    final_specific_date = datetime.combine(specific_date_val, time(0, 0)) # Store as datetime
                ...
                med_data = {
                    ...
                    "days_of_week": days_of_week_val if schedule_type == 'Weekly' else None,
                    "specific_date": final_specific_date # Store None or datetime
                }
                ...
                try:
                    if st.session_state.editing_med_id:
                        ...
                        # Create Pydantic model for validation before saving
                        new_med = Medication(**med_data)
                        add_medication(new_med)
                        st.success(f"Added {med_name}!")

                    clear_form_state_med() # Clear edit state
                    st.rerun() # Rerun to reflect changes and clear form due to clear_on_submit=True
                except Exception as e:
                    st.error(f"Failed to save medication: {e}")

    # Cancel button outside the form, only shown when editing
    if st.session_state.editing_med_id:
            ...
            with st.container(border=True):
                ...
                with p_c3:
                    ...
                    if st.button("Delete", key=f"del_person_{person_id}", type="secondary", use_container_width=True):
                        ...
                        # Clear edit state if implementing edit:
                        # if st.session_state.editing_person_id == person_id: clear_form_state_person()
                        st.rerun()
    st.divider()
    ...
    with st.form(key="person_form", clear_on_submit=True):
        ...
        if person_submitted:
            if person_name and person_relationship:
                ...
                face_encoding_list = None # Initialize as None
                ...
                if uploaded_photo is not None:
                    try:
                        # Use a safe filename based on person's name or a UUID if needed
                        ...
                        photo_url_to_save = str(file_path) # Save the local file path
                        ...
                        with st.spinner("Analyzing face..."):
                            ...
                            if encodings: # Check if list is not empty
                                face_encoding_list = encodings[0].tolist() # Get the first face encoding as a list
                    ...
                    st.warning("No photo uploaded, using placeholder.") # Inform user if no photo provided


                # Create PersonProfile object (without face encoding initially)
                new_person = PersonProfile(
                    name=person_name,
                    relationship=person_relationship,
                    photo_url=photo_url_to_save,
                    notes=person_notes
                    # face_encoding will be added via update if detected
                )

                # Save person to DB
                print(f"--- DEBUG: Attempting to add person: {person_name} ---")
                person_id = add_person(new_person) # add_person should return the new string ID

                # If face was detected AND person was saved successfully, update the record
                if person_id and face_encoding_list:
                    try:
                     ...
                     print(f"--- DEBUG: Person {person_id} added, but no face encoding to save (face not detected or no photo). ---")
                else:
                     print(f"--- DEBUG: Failed to add person {person_name} (already exists or DB error), cannot save encoding. ---")
                     st.warning(f"Could not add {person_name}, possibly already exists.")


                # Don't rerun here, let clear_on_submit handle form clearing
                # st.rerun()
            else:
...
with col3:
    ...
    # Disable button temporarily if PyAudio is not installed
    ...
    if st.button("Start 5-Second Recording", key="record_button", type="secondary", use_container_width=True, disabled=not pyaudio_installed):
        ...
        try:
            with st.spinner("🎤 Recording..."):
                ...
                recorder.cleanup() # Close PyAudio resources

            with st.spinner("🤖 Transcribing..."):
                transcript = transcribe_audio(output_file)
                if not transcript or transcript.startswith("Error:"):
                    raise Exception(f"Transcription failed: {transcript}")
                st.write(f"Transcript: {transcript}") # Show transcript
            ...
            with st.spinner("✨ Summarizing..."):
                ...
                st.write(f"Simple Summary: {simple_summary_text}") # Show summaries

            with st.spinner("💾 Saving..."):
                segment = ConversationSegment(start_time=start_time, end_time=end_time, transcript=transcript)
                # Ensure segment_id is passed correctly (it's generated by Pydantic/PyObjectId now)
                summary_data = {
                    "segment_id": str(segment.id), # Use the generated string ID
                    "simple_summary": simple_summary_text,
                    **clinical_data_dict
                }
                summary = ConversationSummary(**summary_data)
                save_conversation(segment, summary) # Pass Pydantic objects

            st.success("Recording Processed and Saved!")
            st.toast("Refreshing dashboard...")
            # Optionally trigger rerun on other pages if needed via session state flag

        except Exception as e:
            st.error(f"Error during recording/processing: {e}")
            print(f"--- DEBUG: Error in simulation: {e} ---") # Add debug print for errors
        finally:
