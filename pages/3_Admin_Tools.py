...
# --- COLUMN 1: MANAGE MEDICATIONS (Unchanged from last version) ---
with col1:
    ...
    # Function to pre-fill the form state for editing
    ...
    if not medications:
        ...
        for med in medications:
            ...
            with st.container(border=True):
                ...
                with c2:
                    if st.button("Edit", key=f"edit_{med_id}", use_container_width=True, on_click=set_edit_state, args=(med_id,)): pass
                    if st.button("Delete", key=f"del_{med_id}", type="secondary", use_container_width=True):
                        delete_medication(med_id)
                        if st.session_state.editing_med_id == med_id: clear_form_state()
                        st.rerun()
    ...
    with form_col1:
        ...
        if st.button(submit_button_text, type="primary", use_container_width=True):
            ...
            if not is_valid: st.error("Please fill out all medication details.")
            else:
                ...
                try:
                    if st.session_state.editing_med_id:
                        update_medication(st.session_state.editing_med_id, update_data); st.success(f"Updated {st.session_state.med_name}!")
                    else:
                        add_medication(Medication(**update_data)); st.success(f"Added {st.session_state.med_name}!")
                    clear_form_state(); st.rerun()
                except Exception as e: st.error(f"Failed to save to database: {e}")

    with form_col2:
        if st.session_state.editing_med_id:
            if st.button("Cancel", use_container_width=True, on_click=clear_form_state): pass

# --- COLUMN 2: SIMULATE CONVERSATION (THIS IS THE FIX) ---
with col2:
    ...
    if st.button("Start 5-Second Recording", key="record_button", type="secondary", use_container_width=True):
        try:
            # --- The entire process is now wrapped in a try...except block ---
            ...
            with st.spinner("🤖 AI Processing..."):
                ...
                if "error" in clinical_data_dict:
                     raise Exception(f"Clinical summarization failed: {clinical_data_dict['error']}")
            ...
            with st.spinner("💾 Saving to database..."):
                ...
                save_conversation(segment, summary) # This might raise an error if it fails
            
            # This message will ONLY show if all steps above succeed
            st.success("Recording Processed and Saved Successfully!")
            
            # Rerun to refresh the caregiver dashboard immediately
            st.rerun()

        except Exception as e:
            # If any step in the 'try' block fails, this will be shown in the UI
            st.error("An error occurred during the recording process.")
            st.error(f"Details: {e}")

        finally:
            # This will always run, ensuring the temp file is deleted
            try:
